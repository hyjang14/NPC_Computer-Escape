...
-- ↑ 시작화면 배치 ---------------------------------------------------------------------------------------
-------------------------json1에서 정보 읽고 적용↓(임시)-----------------------------------------------------------
	local Data = jsonParse( "liar_round1_json/5.json" )
	...
	local function nextScript( event )
		...
		if wrong == 0 and c == -1 then
			if index == 3 then
				...
				composer.gotoScene("liar_gameround1")
				return
			end
		...
		if i == 1 then    -- 진술 
			...
			if c == 1 and doublecheck == 1 then
				display.remove(dialog)
				composer.setVariable( "heartGroup" , heartGroup)
				composer.gotoScene("liar_gameround1_black")
				return
			elseif wrong == 1 then
				wrong = 0
				i = 0
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene('liar_gameround1_사람5') -- 추가
	elseif phase == "did" then
